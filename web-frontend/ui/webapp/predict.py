...
def predict(data):
	# this dataframe has only one row
	# Only one state, or only homeType
	...
	# needs to tell what our categories are
	...
	prediction_dataframe = pd.get_dummies(prediction_dataframe, columns=['homeType', 'state']) # one hot encoding

	# replace zipcode with the average income 
	# from that zipcode
	# merge income_data to the dataframe by 
	# zip_code (income_data) to zipcode (dataframe)
